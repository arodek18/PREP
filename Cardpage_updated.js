...
export default class Cardpage extends React.Component {
    ...
    async componentDidMount() {
		...
		let available_collectionid;
        this.initialState(_safeguard.cards, navigation.state.params.collectionData.id, navigation.state.params.chartData.id, _safeguard.actions);
        const data = _safeguard.charts.find(item => item.id === navigation.state.params.chartData.id);
		const collection_data = _safeguard.collections.find(item => item.id === navigation.state.params.collectionData.id);
		
		if(data) {
			if(collection_data.title=='Emergency Response Plan')
			{
			available_collectionid = collection_data.id;	
			}else{
			available_collectionid = '';	
			}
            this.setState({
                isIncident: navigation && navigation.state && navigation.state.params && navigation.state.params.safeguard_charts_id === available_collectionid,
                staticData: {
